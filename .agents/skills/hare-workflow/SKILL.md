...
- skills 正本＝`.Codex/skills/hare-*`（本 repo 原生自用）；對外發布靠
  `npm run install-agent`（Codex／cursor／AGENTS.md 三種適配器＋MCP 註冊，見 F3 卡）。
...
- 新專案若 `get_overview.onboarding` 仍存在，這是**執行閘門**：只能讀取、釐清並建立
  Specify／Plan／Tasks 所需的完整卡片與依賴。三步完成後停止，請使用者檢視白板並親自
  勾選「使用者核可」；核可前不得寫程式、安裝依賴或執行實作，也不得由 agent 代銷核可。
...
- **任務封存＝任務紀錄**（不另寫歷史檔）：使用者要求 commit 後，用
  ...
  commit 就直接 `complete_task` 帶 `commit`。AGENTS.md 不堆歷史、不複述白板內容。
