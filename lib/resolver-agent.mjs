...
export function spawnResolver({ execCommand, cwd, prompt, timeoutMs = 300000 } = {}) {
  return new Promise((res) => {
    ...
    let c;
    try {
      // .cmd／.bat 需 shell；shell 模式傳單一字串，不配 args 陣列（DEP0190）
      const viaShell = process.platform === "win32" && /\.(cmd|bat)$/i.test(cmd);
      c = viaShell
        ? spawn([cmd, ...args].join(" "), { cwd, windowsHide: true, shell: true, stdio: ["pipe", "pipe", "pipe"] })
        : spawn(cmd, args, { cwd, windowsHide: true, stdio: ["pipe", "pipe", "pipe"] });
